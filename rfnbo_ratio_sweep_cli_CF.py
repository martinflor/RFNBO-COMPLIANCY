"""Standalone RFNBO Energy Ratio Sweep CLI.
...
### Sizing Logic (Energy-based via Capacity Factors)
Unlike MW-based models, the ratio in this script dictates the **annual energy volume (MWh)**. 
The script dynamically calculates Capacity Factors (CFs) from historical ENTSO-E data 
to properly size the physical MW required to hit your target energy ratio.
* A ratio of 1.0 means: `Total PPA Annual Energy = Electrolyser Annual Energy Demand`.
* For combined/hybrid technologies (e.g., "Solar + Wind Offshore"), the script assumes 
  a 50/50 **energy** split. Because solar typically has a lower CF than wind, achieving 
  this equal energy split will result in building significantly more Solar MW than Wind MW.
...
2.  **Summary Plot (`*_ratio_sweep.html`):** An interactive line chart displaying the 
    Production-to-Consumption Energy Ratio (X-axis) against the overall RFNBO % (Y-axis), 
...
> python rfnbo_energy_ratio_sweep_cli.py --country Belgium --electrolyser-mw 50
...
> python rfnbo_energy_ratio_sweep_cli.py \
    --country Germany \
    --electrolyser-mw 100 \
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "export CSV-only interval RFNBO percentages, using Capacity Factors for sizing."
        )
    )
    ...
    parser.add_argument(
        ...
        help="PPA-to-electrolyser energy ratios to sweep (default: 200 points from 0.0 to 2.0)",
    )
    parser.add_argument(
        "--technologies",
        nargs="+",
...
def _sweep_technology(
    ...
    # Calculate required capacities based on target energy ratio and CFs
    if "+" in technology_lower:
        wind_key = "offshore" if "offshore" in technology_lower else "onshore"
        
        # Assume 50/50 ENERGY split for combined technologies
        solar_energy_share = 0.5
        wind_energy_share = 0.5
        
        solar_cf = cfs.get("solar", DEFAULT_CFS["solar"])
        wind_cf = cfs.get(wind_key, DEFAULT_CFS[wind_key])

        solar_capacity_mw = electrolyser_mw * ratio * solar_energy_share / solar_cf
        wind_capacity_mw = electrolyser_mw * ratio * wind_energy_share / wind_cf
        ppa_capacity_mw = solar_capacity_mw + wind_capacity_mw
        
        # Calculate capacity fractions for the compliance engine
        if ppa_capacity_mw > 0:
            solar_fraction = solar_capacity_mw / ppa_capacity_mw
            wind_fraction = wind_capacity_mw / ppa_capacity_mw
        else:
            solar_fraction, wind_fraction = 0.5, 0.5
            
    else:
        if "solar" in technology_lower:
            cf = cfs.get("solar", DEFAULT_CFS["solar"])
        elif "offshore" in technology_lower:
            cf = cfs.get("offshore", DEFAULT_CFS["offshore"])
        else:
            cf = cfs.get("onshore", DEFAULT_CFS["onshore"])
            
        ppa_capacity_mw = (electrolyser_mw * ratio) / cf
        solar_fraction, wind_fraction = None, None
...
def main() -> None:
    ...
    # Calculate Capacity Factors dynamically from the data
    ...
    print("\nCapacity factors used (calculated from ENTSOE data):")
    print(f"  - Solar: {cfs['solar']:.4f}")
    print(f"  - Offshore Wind: {cfs['offshore']:.4f}")
    print(f"  - Onshore Wind: {cfs['onshore']:.4f}\n")
    ...
    for technology in args.technologies:
        for ratio_index, ratio in enumerate(args.ratios, start=1):
            if progress_every and (ratio_index == 1 or ratio_index % progress_every == 0 or ratio_index == total_ratios):
                print(f"[{technology}] energy ratio {ratio_index}/{total_ratios}: {ratio:.6f}")
